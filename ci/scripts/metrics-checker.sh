...
checkServiceYAML(){
    SERVICE_NAME=$1
    if grep -q " include \"$SERVICE_NAME.annotations\" " ./charts/$SERVICE_NAME/templates/service.yaml; then
...
checkConfigMapYAML(){
    SERVICE_NAME=$1
    if grep -q " include \"$SERVICE_NAME.prometheus\" " ./charts/$SERVICE_NAME/templates/configmap.yaml; then
...
checkHelperTPL(){
    ...
    echo -e "prometheus.io/scrape-role: {{ .Values.prometheus.scrapeRole | quote }}" >> .bob/var.compareToLine;
